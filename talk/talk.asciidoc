...
.Control Constructs per Lines of Code in Each Sample
image::images/constructs_loc_per_year.png[]

Now, the first interesting thing we can notice is that the ratio of control constructs to lines of code is declaining. This teaches us that most new or refactored code is not control flow constructs or looping functions. This shouldn't suprise us, control flow constructs are only a certain percent of the code.
